...
def main():
    ...
    if len(args) != 3:
        print("Usage: python sisr_auto_s2albedo_single_band.py <input> <out_dir> <weights> [-COG] [-CLEAN]")
        sys.exit(1)
    inp, out_dir, weights = args

    overall_start = time.time()

    print("### Sentinel-2 Albedo Single-Band Super-Resolution Pipeline ###")
    ...
    if not pretrained_base.startswith("m-4"):
        print("Model mismatch for M-4: expected weights trained for Sentinel-2 albedo imagery.")
        sys.exit(1)

#!/usr/bin/env python3
"""
Sentinel‑2 RGB super‑resolution automation
-----------------------------------------
This is an updated version of the original **sisr_auto_sentinel‑2_rgb.py** script.

Changes compared with the original:
1. Clearer, more structured command‑line messages (no arrows).
2. Real‑time progress indicator when running on tiles (e.g. *Processing tile 3/17 …*).
3. Accurate timing — the script reports
   • time spent inside *inference.py* (cumulative if tiles are used), and
   • total wall‑clock processing time for the whole pipeline.
4. New optional flag **-CLEAN** – when supplied, all intermediate files
   (per‑tile outputs and temporary PNG / header files) are removed at the end.

Usage:
    python sisr_auto_sentinel-2_rgb.py <input> <out_dir> <weights> [-COG] [-CLEAN]

The script still relies on *inference.py* for the actual super‑resolution step
...
def main() -> None:
    if len(sys.argv) < 4:
        print("Usage: python sisr_auto_sentinel-2_rgb.py <input> <out_dir> <weights> [-COG] [-CLEAN]")
        sys.exit(1)
    ...
    _log("Starting Sentinel‑2 RGB super‑resolution pipeline")
    ...
    if not (wb.startswith("m-1")):
        print("Error: model mismatch for M-1: only weights trained for Sentinel‑2 RGB imagery are supported")
        sys.exit(1)
